...
public class ExamPublishingService
{
    ...
    private string GetExamJavaScript(ExamReviewData examData, string examId)
    {
        ...
        sb.AppendLine("            var connectedState = (typeof signalR !== 'undefined' && signalR.HubConnectionState) ? signalR.HubConnectionState.Connected : 'Connected';");
        sb.AppendLine("            var disconnectedState = (typeof signalR !== 'undefined' && signalR.HubConnectionState) ? signalR.HubConnectionState.Disconnected : 'Disconnected';");
        sb.AppendLine("            if (window.signalRConnection.state === connectedState) {");
        sb.AppendLine("                return invokeNow();");
        sb.AppendLine("            }");
        sb.AppendLine("            if (window.signalRConnection.state === disconnectedState) {");

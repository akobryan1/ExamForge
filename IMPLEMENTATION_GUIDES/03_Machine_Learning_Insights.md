# Machine Learning Insights Implementation Guide

## Overview
Add predictive analytics and intelligent recommendations using ML.NET.
...
Add to `ExamForge/Services/MachineLearningService.cs`:
...
namespace ExamForge.Services
{
    public class MachineLearningService
    {
            ...
            PublishedExam exam)
        {
            ...
            var questionGroups = exam.Contents.GroupBy(c => c.Topic ?? "General");
...
## Step 3: Add ML Insights Panel to Analytics

Add a new tab to `student_analytics_usercontrol.xaml`:

```xaml
<RadioButton x:Name="MLInsightsTab"
             Content="?? ML Insights"
             Style="{StaticResource AnalyticsTabButtonStyle}"
             Checked="AnalyticsTab_Changed"/>
```

## Step 4: Create ML Insights Panel

```xaml
<StackPanel x:Name="MLInsightsPanel" Visibility="Collapsed">
    <!-- At-Risk Students -->
    <Border Style="{StaticResource AnalyticsCardStyle}" Margin="0,0,0,16">
        <StackPanel>
            <TextBlock Text="?? At-Risk Students" FontSize="16" 
                       FontWeight="SemiBold" Margin="0,0,0,12"/>
            <DataGrid x:Name="AtRiskGrid" AutoGenerateColumns="False" 
                      Height="200">
                <DataGrid.Columns>
                    <DataGridTextColumn Header="Student" Binding="{Binding StudentName}"/>
                    <DataGridTextColumn Header="Risk Level" Binding="{Binding RiskLevel}"/>
    ...
    <Border Style="{StaticResource AnalyticsCardStyle}" Margin="0,0,0,16">
        <StackPanel>
            <TextBlock Text="?? Identified Learning Gaps" FontSize="16" 
                       FontWeight="SemiBold" Margin="0,0,0,12"/>
            <ItemsControl x:Name="LearningGapsContainer"/>
        </StackPanel>
    </Border>

    <!-- Difficulty Recommendation -->
    <Border Style="{StaticResource AnalyticsCardStyle}">
        <StackPanel>
            <TextBlock Text="?? Exam Difficulty Recommendation" FontSize="16" 
                       FontWeight="SemiBold" Margin="0,0,0,12"/>
            <TextBlock x:Name="DifficultyRecommendationText" TextWrapping="Wrap"/>
        </StackPanel>
    </Border>
</StackPanel>
```
...
## Benefits: Predictive analytics, early intervention, personalized recommendations
## Dependencies: ML.NET
